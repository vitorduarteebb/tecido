...
const mapearCamposCliente = (row) => {
    ...
    const findField = (possibleNames) => {
        for (const name of possibleNames) {
            const found = campos.find(campo => campo.toLowerCase().includes(name.toLowerCase()) ||
...
async function debugImportClientes() {
    try {
        ...
        for (let i = 0; i < Math.min(data.length, 10); i++) { // Limitar a 10 linhas para debug
            ...
            const isHeaderOrTitle = campos.razaoSocial === 'Emitido em 04/07/2025' ||
