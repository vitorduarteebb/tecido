...
exports.pedidoController = {
    criar: async (req, res) => {
        try {
            ...
            console.log('[pedidoController] Dados extraídos:', {
                itens: itens === null || itens === void 0 ? void 0 : itens.length,
                condicaoPagamento,
                detalhePrazo,
            ...
            const itensAtualizados = await Promise.all(itens.map(async (item, index) => {
            ...
            const pedido = await models_1.Pedido.create(Object.assign(Object.assign({}, req.body), { itens: itensAtualizados, valorTotal: valorTotalPedido, pesoTotal: pesoTotalPedido, status: 'Em Separação', detalhePrazo: condicaoPagamento === 'aprazo' ? detalhePrazo : undefined, numeroPedido,
                clienteId, representanteId: idRepresentante }));
    ...
    dashboard: async (req, res) => {
        try {
            ...
            // Pedidos pendentes
            const pedidosPendentes = await models_1.Pedido.count({
                where: { status: { [sequelize_1.Op.in]: ['pendente', 'Aguardando Aprovação'] } }
            });
            // Pedidos entregues
