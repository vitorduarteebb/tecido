...
export const importClientes = async (req: Request, res: Response) => {
  try {
    if (!req.file) {
    ...
    for (let i = 0; i < rawData.length; i++) {
      ...
      if (row && row.some((cell: any) => 
        cell && cell.toString().toLowerCase().includes('razão social') ||
...
export const importProdutos = async (req: Request, res: Response) => {
  try {
    if (!req.file) {
      return res.status(400).json({ error: 'Nenhum arquivo enviado' });
    }
